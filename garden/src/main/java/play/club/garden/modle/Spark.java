package play.club.garden.modle;
...
//    }
    ...
    public Spark(int pWidth, int pHeight) {
        // 初始化画笔
        setSparkPaint();
        this.mWidth = pWidth;
    ...
    public int[] drawSpark(Canvas canvas, int x, int y, int[] store) {
        ...
        if (mCurDistance == mDistance && isActive) {
            mDistance = getRandom(mWidth / 4, mRandom.nextInt(15)) + 1;
            mCurDistance = 0;

            start = new Point(X, Y);
            end = getRandomPoint(start.x, start.y, (int) mDistance);
            c1 = getRandomPoint(start.x, start.y, mRandom.nextInt(mWidth / 16));
            c2 = getRandomPoint(end.x, end.y, mRandom.nextInt(mWidth / 16));
        }
        // 恢复火花路径
        else {
